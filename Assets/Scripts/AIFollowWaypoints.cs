...
public class AIFollowWaypoints : MonoBehaviour, IAIPackage
{
    ...
    private void followWaypoints()
    {
        if(waypoints.Count > 0) //are there any waypoints?
        {
            ...
            {
                controller.creature.lookAt(waypoints[0].position); //rotate the ship towards the waypoint
                controller.creature.speedUp(); //move the ship towards the waypoint
            }
    ...
    private void patrolWaypoints()
    {
        if (waypoints.Count > 0) //are there any waypoints?
        {
            ...
            if (distanceToWaypoint > minWaypointDistance) //did we reach the waypoint?
            {
                controller.creature.lookAt(currentWaypoint.position); //rotate the ship towards the waypoint
                controller.creature.speedUp(); //move the ship towards the waypoint
            }
            else //in case we reached the waypoint
            {
